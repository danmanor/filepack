...
def test_gzip_compression_in_place(txt_file: Path):
    comp = Compression(txt_file)

    uncompressed_size_before = txt_file.stat().st_size
    comp.compress(compression_algorithm="gz")  
    assert comp.is_compressed(compression_algorithm="gz")

    compressed_size = Path(str(txt_file) + ".gz").stat().st_size
...
def test_bz2_compression_in_place(txt_file: Path):
    comp = Compression(txt_file)

    uncompressed_size_before = txt_file.stat().st_size
    comp.compress(compression_algorithm="bz2")  
    assert comp.is_compressed(compression_algorithm="bz2")

    compressed_size = Path(str(txt_file) + ".bz2").stat().st_size
...
def test_lz4_compression_in_place(txt_file: Path):
    comp = Compression(txt_file)

    uncompressed_size_before = txt_file.stat().st_size
    comp.compress(compression_algorithm="lz4")  
    assert comp.is_compressed(compression_algorithm="lz4")

    compressed_size = Path(str(txt_file) + ".lz4").stat().st_size
...
def test_xz_compression_in_place(txt_file: Path):
    comp = Compression(txt_file)

    uncompressed_size_before = txt_file.stat().st_size
    comp.compress(compression_algorithm="xz")  
    assert comp.is_compressed(compression_algorithm="xz")

    compressed_size = Path(str(txt_file) + ".xz").stat().st_size
